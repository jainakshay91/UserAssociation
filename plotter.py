...
def hmap_creator(usr_lcs, mc_lcs, sc_lcs, rate_mat, connect_info_mat, np, scn):
	...
	s1, = ax.plot(usr_lcs[:,0], usr_lcs[:,1], "r*", markersize=6) # Plot the User locations
	...
	plt.legend([s1,s2,s3],["Users","Macro Cells", "Small Cells"],loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol = 3) 
	plt.xticks([])
	plt.yticks([])
	#plt.title("Heatmap of Individual User Data Rates (DC+MRT Scenario)")
	plt.savefig('Topology', dpi=1200, facecolor='w', edgecolor='w',
...
def hist_plotter(rate_matrix_DC, rate_matrix_SA, rate_matrix_DC_MRT_BHCAP, rate_matrix_DC_BHCAP, rate_matrix_SA_BHCAP, rate_matrix_SA_LAT, rate_matrix_SA_MRT, rate_matrix_DC_MRT, rate_matrix_DC_LAT, rate_matrix_SA_MRT_LAT, rate_matrix_DC_MRT_LAT, rate_matrix_SA_BHCAP_LAT, rate_matrix_DC_BHCAP_LAT, np,scn):
	...
	ax1.plot(bins3[0]/1e9, n3[0], 'ko', markersize=6, label="Minimum Rate with DC+MRT: "+str(format(bins3[0]/1e6,'0.2f'))+" Mbps")
	...
	# axins.yaxis.set_ticks(np.arange(0.03,0.065,0.01))
	...
	plt.savefig('UserDistCDF', dpi=1200, facecolor='w', edgecolor='w',
        orientation='landscape', papertype='letter', format='png',
        transparent=False, bbox_inches='tight', pad_inches=0.1,
        frameon=None, metadata=None)
...
def bhutil_latprov_plotter(bhutil_val_DC, bhutil_val_DC_MRT_BHCAP, bhutil_val_DC_BHCAP, bhutil_val_DC_BHCAP_LAT, avail_bh, latprov_DC, latprov_DC_LAT, latprov_DC_MRT_LAT, latprov_DC_BHCAP_LAT, np, scn):
	...
	print ("For DC + CB Scenario:")
	print ("MC average BH Consumpution:", np.mean(np.array(bhvbh)[len(avail_bh):]))
	print ("SC average BH Consumpution:", np.mean(np.array(bhvbh)[:len(avail_bh)]))
	print ("===================================================")
	
	# bhvbh_MRT = [item for sublist in bhutil_val_DC_MRT_BHCAP for item in sublist]
	# print ("For DC + CB + MRT Scenario:")
	# print ("MC average BH Consumpution:", np.mean(np.array(bhvbh_MRT)[len(avail_bh):]))
	# print ("SC average BH Consumpution:", np.mean(np.array(bhvbh_MRT)[:len(avail_bh)]))
	...
	for i in range(0, len(bhvbh)-len(avail_bh), 2):
		#print i 
		#print ("Idx:",len(bhvbh)-1-i)
		ax.get_xticklabels()[len(ax.get_xticklabels())-1 - i/2].set_color("red") 

	# for i in range(0, len(avail_bh), 2):
	# 	if avail_bh[i] == 1e9:
	# 		ax.get_xticklabels()[i/2].set_color("green") 
	...
	fig.tight_layout()
	plt.savefig('BHUTIL', dpi=1200, facecolor='w', edgecolor='w',
        orientation='landscape', papertype='letter', format='png',
        transparent=False, bbox_inches='tight', pad_inches=0.1,
        frameon=None, metadata=None)
	...
	ax1.scatter(np.arange(1,lprov_DC_LAT.shape[0]+1), lprov_DC_LAT[:,0], c = 'b', marker = 'o',  alpha = 0.5)
	ax1.scatter(np.arange(1,lprov_DC_LAT.shape[0]+1), lprov_DC_LAT[:,1], c = 'b', marker = 'o',  alpha = 0.5)
	ax1.plot(np.arange(latprov_DC_LAT.shape[0]), [scn.eMBB_latency_req]*latprov_DC_LAT.shape[0],'r-')
	ax1.set_ylim(0,0.004)
	plt.xlabel('Users')
	plt.ylabel('Latency (in seconds)')
	#plt.title("Latency Scatter Plot of Users (DC LAT Scenario)")
	plt.legend(["3 ms"])
	plt.savefig('LATDC', dpi=1200, facecolor='w', edgecolor='w',
        orientation='landscape', papertype='letter', format='png',
        transparent=False, bbox_inches='tight', pad_inches=0.1,
        frameon=None, metadata=None)
...
def infeasible_iter_counter(iters_infeas, iters_infeas_DC, iters_infeas_DC_MRT, iters_infeas_SA_MRT_LAT, iters_infeas_SA_BHCAP_MRT, iters_infeas_DC_BHCAP_MRT, iters_infeas_DC_BHCAP_MRT_LAT, iters_infeas_SA_MRT , iters_timeout, iters_timeout_DC, iters_timeout_DC_MRT, iters_timeout_SA_MRT_LAT, iters_timeout_SA_BHCAP_MRT, iters_timeout_DC_BHCAP_MRT, iters_timeout_DC_BHCAP_MRT_LAT, iters_timeout_SA_MRT , iters_infeas_SA_MRT_BHCAP_LAT, iters_timeout_SA_MRT_BHCAP_LAT, iters_infeas_DC_MRT_LAT,iters_timeout_DC_MRT_LAT, iters_infeas_SA_BHCAP, iters_timeout_SA_BHCAP, iters_infeas_SA_LAT, iters_timeout_SA_LAT,
		...
		iters_infeas_DC_BHCAP_LAT, iters_timeout_DC_BHCAP_LAT, np,scn, idx):
	...
	plt.savefig('IterInfeas'+str(idx), dpi=1200, facecolor='w', edgecolor='w',
        orientation='landscape', papertype='letter', format='png',
        transparent=False, bbox_inches='tight', pad_inches=0.1,
        frameon=None, metadata=None)
# ===========
...
def timecdf(time_DC, time_DC_MRT , time_SA_MRT , time_DC_BHCAP , time_DC_BHCAP_LAT , time_DC_LAT , time_SA_BHCAP , time_SA_BHCAP_LAT , time_SA_LAT ,time_SA, np, scn ):
	#plt.close('all')
	#print time_SA_MRT[:,5]
	n_bins = 10000; # Number of bins for the histogram
	...
	n2, bins2, patches2 = axs.hist(time_DC_BHCAP[:,4], n_bins,density=True, histtype='step',
	...
	n4, bins4, patches4 = axs.hist(time_DC_BHCAP_LAT[:,4], n_bins,density=True, histtype='step',
	...
	n6, bins6, patches6 = axs.hist(time_DC_LAT[:,4], n_bins,density=True, histtype='step',
                           cumulative=True, label='Empirical', color='g')
	...
	n8, bins8, patches8 = axs.hist(time_SA_LAT[:,4], n_bins,density=True, histtype='step',
                           cumulative=True, label='Empirical', color='g')
	
	seq_bins8 = np.array([bins8[0]])
	n8 = np.concatenate((seq,n8), axis =0)
	bins8 = np.concatenate((seq_bins8,bins8), axis =0)
	
	n9, bins9, patches9 = axs.hist(time_SA_BHCAP[:,4], n_bins,density=True, histtype='step',
                           cumulative=True, label='Empirical', color='g')
	
	seq_bins9 = np.array([bins9[0]])
	n9 = np.concatenate((seq,n9), axis =0)
	bins9 = np.concatenate((seq_bins9,bins9), axis =0)

	n10, bins10, patches10 = axs.hist(time_SA_BHCAP_LAT[:,4], n_bins,density=True, histtype='step',
                           cumulative=True, label='Empirical', color='g')
	
	seq_bins10 = np.array([bins10[0]])
	n10 = np.concatenate((seq,n10), axis =0)
	bins10 = np.concatenate((seq_bins10,bins10), axis =0)
	...
	plt.close()
	f, ax1 = plt.subplots()
	ax1.plot(bins1[:-1],n1,'r--', label="DC")
	ax1.plot(bins5[:-1],n5,'c--', label="SA")
	ax1.plot(bins7[:-1],n7,'#bfbe7b', label="SA+MRT", linestyle = '-.')
	ax1.plot(bins2[:-1],n2,'k--', label="DC+CB")
	ax1.plot(bins3[:-1],n3,'#ea207a', label="DC+MRT", linestyle = '-.')
	ax1.plot(bins6[:-1],n6,'m--', label="DC+CPL")
	#ax1[1].plot(bins6[:-1],n6,'m-', label="DC+LAT")
	ax1.plot(bins4[:-1],n4,'#ffa07a', label="DC+CB+CPL", linestyle = '--')
	ax1.plot(bins8[:-1],n8,'g--', label="SA+CPL")
	ax1.plot(bins9[:-1],n9,'b--', label="SA+CB" )
	ax1.plot(bins10[:-1],n10,'#b22222', label="SA+CB+CPL", linestyle = '--')
	...
	ax1.set_xlim(min(min(bins3), min(bins7),min(bins4), min(bins10), min(bins9), min(bins1), min(bins5), min(bins2), min(bins6), min(bins8))-0.1, max(max(bins10), max(bins2), max(bins4), max(bins9), max(bins1), max(bins5), max(bins6), max(bins8))+0.1)
	ax1.set_ylim(0,1)
	ax1.grid(alpha = 0.2, linestyle = '--')
	ax1.set_xlabel("Processing time (in seconds)")
	...
	plt.savefig('ProcTimeCDF', dpi=1200, facecolor='w', edgecolor='w',
        orientation='landscape', papertype='letter', format='png',
        transparent=False, bbox_inches='tight', pad_inches=0.1,
        frameon=None, metadata=None)
